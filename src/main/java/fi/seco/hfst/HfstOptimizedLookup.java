...
public class HfstOptimizedLookup {
	...
	public static void runTransducer(Transducer t) {
		...
		while (true) {
			...
			Collection<Result> analyses = t.analyze(str);
			for (Result analysis : analyses)
				System.out.println(str + "\t" + analysis);
			if (analyses.isEmpty()) System.out.println(str + "\t+?");
			System.out.println();
		}
	}
	...
	public static void main(String[] argv) throws IOException {
		...
		if (h.isWeighted()) {
			Transducer transducer = new WeightedTransducer(charstream, h, a);
			runTransducer(transducer);
		} else {
			Transducer transducer = new UnweightedTransducer(charstream, h, a);
			runTransducer(transducer);
		}
	}
}